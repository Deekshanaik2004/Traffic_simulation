...
STARVE_TIME = 25.0
SAFE_DISTANCE = 45
SPAWN_CHANCE = 20  # the lower, the more often vehicles spawn (random modulus)
...
def draw_vehicle(screen, x, y, direction, sprite_type, vehicle_length, vehicle_width):
    """
    Draws a top-down rectangle vehicle oriented according to direction.
    x,y are top-left coordinates in the way existing code expects.
    """
    if sprite_type == 'car':
        body_color = (0, 0, 255)
...
class Car:
    ...
    def get_front_car(self, cars):
        ...
        if self.direction == "N":
            ...
            # FIXED: proper filtering for eastbound
...
def main_loop():
    ...
    while True:
        ...
        # update cars list
        # (we perform rendering from new_cars below)
        ...
        if light_state == "GREEN":
            ...
            # move to WAIT_CLEAR state (light is effectively red for others)
            light_state = "WAIT_CLEAR"
            clear_start_time = time.time()

        elif light_state == "WAIT_CLEAR":
            # check virtual IoT sensors
